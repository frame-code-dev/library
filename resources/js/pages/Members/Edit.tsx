...
export default function MemberEdit({ member }: { member: any }) {
    const { data, setData, put, processing, errors } = useForm({
        name: member.name,
        email: member.email,
        gender: member.gender || 'male',
        type: member.type,
        class_or_position: member.class_or_position,
    });
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 w-full">
                <Card>
                    <CardHeader className="py-4">
                        <CardTitle className="text-lg font-semibold">Edit Anggota: {member.name}</CardTitle>
                    </CardHeader>
                    <CardContent>
